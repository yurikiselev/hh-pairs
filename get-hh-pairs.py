...
# блок констант
UTF8 = 'utf-8'
mark_words = ["женск".decode(UTF8), "ласк".decode(UTF8),
    "уменьш".decode(UTF8)]
intro_beg = ["в".decode(UTF8), "во".decode(UTF8), "на".decode(UTF8),
    "по".decode(UTF8), "у".decode(UTF8)]
...
class TWord:
    ...
    def to_string(self):
        return (self.lemma + " {" + self.form + "}").encode(UTF8)
...
def is_syn_def(definition):
    ...
    return w1 == "то".decode(UTF8) and w2 == "же".decode(UTF8) \
            and w3 == "что".decode(UTF8)
...
def is_ref_def(definition):
    ...
    w1, pos = get_next_word_lower(definition, pos)
    w2, pos = get_next_word_lower(definition, pos)
    if (w1 == "действие".decode(UTF8) \
            or w1 == "свойство".decode(UTF8) \
            or w1 == "состояние".decode(UTF8)) \
            and w2 == "по".decode(UTF8):
        return True
    w3, pos = get_next_word_lower(definition, pos)
    if (w1 == "действие".decode(UTF8) \
            or w1 == "свойство".decode(UTF8)) \
            and w2 == "и".decode(UTF8) \
            and w3 == "состояние".decode(UTF8):
        return True
    return False
...
def is_person(definition):
    ...
    w1, pos = get_next_word_lower(definition, pos)
    w2, pos = get_next_word_lower(definition, pos)
    return w1 == "тот".decode(UTF8) and w2 == "кто".decode(UTF8)
...
def print_def(origin_line):
    ...
    sys.stdout.write(cut_line.encode(UTF8))
...
def is_nomin_case(gr):
    return gr.find("им".decode(UTF8)) != -1
...
if __name__ == "__main__":
    for line in sys.stdin:
        ...
        origin_line = origin_line.decode(UTF8)
        ...
        if is_person(definition):
            print_def_and_pairs(hypernym, TWord("человек".decode(UTF8),
                    "человек".decode(UTF8)), TWord(), origin_line)
            continue
        ...
        while fn < len(definition):
            if is_part_of_word(definition[fn]):
                ...
                if lower_form == "название".decode(UTF8):
                    name = True
                    continue
                else:
